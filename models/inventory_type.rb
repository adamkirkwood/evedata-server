class InventoryType < ActiveRecord::Base
  ...
  default_scope { select("invTypes.typeID, invTypes.groupID, invTypes.typeName, invTypes.basePrice, invTypes.volume, invTypes.mass, invTypes.capacity, invTypes.description, invGroups.groupName") }
  
  scope :by_id, lambda { |value| where("typeID = ?", value) if value }
  scope :by_name, lambda { |value| where("typeName = ?", "#{value}") if value }
  
  def as_json(options={})
    options[:methods] = [:id, :name, :images, :group]
    options[:only] = [:id, :name, :base_price, :volume, :mass, :capacity, :description, :group]
    super
  end
